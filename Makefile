...
proof: venv build.stamp
	...
	TOCHECK=$$(find fonts/variable -type f 2>/dev/null); if [ -z "$$TOCHECK" ]; then TOCHECK=$$(find fonts/ttf -type f 2>/dev/null); fi ; (for FONT in $$TOCHECK; do mkdir -p out/`basename $$FONT .ttf`/proof; diff3proof $$FONT --output out/`basename $$FONT .ttf`/proof; done)
